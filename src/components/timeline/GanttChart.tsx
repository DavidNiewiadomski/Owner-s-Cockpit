...
export function GanttChart({ data }: GanttChartProps) {
  ...
  const maxWeek = Math.max(...data.map(item => Math.max(item.plannedEnd, item.actualEnd || 0))) + 5;
      ...
      <CardContent>
        <div className="relative overflow-x-auto">
          ...
          <div className="flex border-b mb-2 pb-2">
            ...
            <div className="flex-1 flex">
              {Array.from({ length: maxWeek }).map((_, i) => (
                <div 
                  key={i} 
                  className={`text-xs text-center ${i === 18 ? 'text-red-500 font-bold' : ''}`}
                  style={{ width: '20px' }}
                >
                  {i}
                </div>
          ...
          <div className="absolute top-8 bottom-0 border-l-2 border-red-500 z-10"
            style={{ 
              left: `calc(40px + (18 * 20px))`,
              borderStyle: 'dashed'
            }}>
            <div className="absolute -top-0 -left-12 bg-red-100 text-red-800 px-1 rounded text-xs whitespace-nowrap">
              Today
            </div>
          </div>
              ...
              <div key={index} className="flex items-center">
                ...
                <div className="flex-1 relative h-8" style={{ minWidth: `${maxWeek * 20}px` }}>
                  ...
                  {Array.from({ length: maxWeek }).map((_, i) => (
                    <div 
                      key={i} 
                      className={`absolute top-0 bottom-0 border-l ${i % 5 === 0 ? 'border-gray-300' : 'border-gray-200'}`}
                      style={{ left: `${i * 20}px` }}
                    />
                  ))}
                  ...
                  {/* Planned bar */}
                  <div 
                    className="absolute h-6 rounded-md bg-blue-500 opacity-60 flex items-center justify-center z-10"
                    style={{ 
